...
var LOG_PREFIX = 'parse-push-adapter-web GCM';
...
function generateGCMPayload(requestData, pushId, timeStamp, expirationTime) {
  _npmlog2.default.verbose(LOG_PREFIX, 'GCM generateGCMPayload');
  var payload = {
    priority: 'high'
  };
  payload.data = {
  ...
  _npmlog2.default.verbose(LOG_PREFIX, 'GCM requestData %s', JSON.stringify(requestData, null, 4));

  optionalKeys.forEach(function (key, index, array) {
    if (requestData.hasOwnProperty(key)) {
      _npmlog2.default.verbose(LOG_PREFIX, 'GCM found optional key %s', key);
